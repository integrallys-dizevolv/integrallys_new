...
import { useFormasPagamento } from '@/features/formas-pagamento/hooks/use-formas-pagamento'
...
export function NovoLancamentoModal({ open, onClose, form, onChange, onSave }: Props) {
  ...
  const contasAtivas = contas.filter((item) => item.ativo)
  const formaSelecionada = formasAtivas.find((item) => item.id === form.formaPagamentoId)
  const mostrarParcelas = formaSelecionada?.tipo === 'cartao_credito'

  return (
    <Dialog open={open} onOpenChange={(nextOpen) => !nextOpen && onClose()}>
      <DialogContent
        size="lg"
        className="gap-0 rounded-[24px] border border-app-border p-0 dark:border-app-border-dark"
      >
            ...
            <div className="mt-8 grid grid-cols-1 gap-6 md:grid-cols-2">
              ...
              <div className="space-y-3">
                ...
                <div className="relative">
                  ...
                  <Input
                    value={form.data}
                    onChange={(event) => onChange({ ...form, data: event.target.value })}
              ...
              <div className="space-y-3 md:col-span-2">
                ...
                <Input
                  ...
                  onChange={(event) => onChange({ ...form, beneficiario: event.target.value })}
              ...
              <div className="space-y-3">
                ...
                <Select
                  value={form.formaPagamentoId || 'none'}
                  onValueChange={(value) =>
                    onChange({
                      ...form,
                      formaPagamentoId: value === 'none' ? '' : value,
                      parcelas: 1,
                    })
                  }
                >
              ...
              <div className="space-y-3">
                ...
                <Select
                  ...
                  onValueChange={(value) =>
                    onChange({ ...form, contaBancariaId: value === 'none' ? '' : value })
                  }
                >
                  <SelectTrigger className="h-12 rounded-[12px]">
                    <SelectValue placeholder="Selecione" />
              ...
              {mostrarParcelas && (
                <div className="space-y-3">
                  ...
                  <Input
                    ...
                    value={form.parcelas}
                    onChange={(event) =>
                      onChange({ ...form, parcelas: Math.max(1, Number(event.target.value) || 1) })
                    }
                    className="h-12 rounded-[12px]"
                  />
                </div>
