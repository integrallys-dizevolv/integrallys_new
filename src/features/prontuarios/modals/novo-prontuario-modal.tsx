...
import type { NovoProntuarioInput } from '@/features/prontuarios/hooks/use-prontuarios'
...
export function NovoProntuarioModal({
  isOpen,
  onClose,
  pacientes,
  isLoadingPacientes,
  onSave,
}: NovoProntuarioModalProps) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      toast.error(err instanceof Error ? err.message : 'Não foi possível criar o prontuário.')
    } finally {
      setIsSaving(false)
    }
  }
  ...
  return (
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <DialogContent
        ...
        className="sm:max-w-[650px] p-0 rounded-[20px] overflow-hidden border border-app-border dark:border-app-border-dark shadow-lg bg-app-card dark:bg-app-card-dark"
      >
        <DialogTitle className="sr-only">Novo prontuário</DialogTitle>
        <div className="bg-app-card dark:bg-app-card-dark p-10 custom-scrollbar">
          <div className="flex justify-between items-start mb-10">
            <div className="space-y-1.5">
              ...
              <p className="text-app-text-secondary dark:text-app-text-muted text-base font-normal">
          ...
          <div className="space-y-8">
            <div className="space-y-3">
              ...
              <Select value={pacienteId} onValueChange={setPacienteId}>
                <SelectTrigger className="h-12 rounded-[12px] border border-app-border dark:border-app-border-dark bg-app-card dark:bg-app-card-dark px-4 text-app-text-primary dark:text-white font-normal focus:ring-[var(--app-card-dark)]">
                  <SelectValue preferPlaceholder placeholder={isLoadingPacientes ? 'Carregando pacientes...' : 'Selecione o paciente'} />
                </SelectTrigger>
            ...
            <div className="space-y-3">
              <Label className="text-base font-normal text-app-text-primary dark:text-white tracking-tight">
                Data do atendimento <span className="text-[var(--app-danger-text)]">*</span>
              </Label>
            ...
            <div className="space-y-3">
              <Label className="text-base font-normal text-app-text-primary dark:text-white tracking-tight">
                Tipo de consulta <span className="text-[var(--app-danger-text)]">*</span>
              </Label>
            ...
            <div className="flex items-center justify-end gap-3 pt-6">
              ...
              <Button
                className="h-12 px-10 rounded-[12px] bg-app-primary hover:bg-app-primary-hover text-white font-normal text-base shadow-sm transition-all"
                onClick={() => void handleSave()}
                disabled={isSaving || isLoadingPacientes}
              >
                {isSaving ? 'Criando...' : 'Criar prontuário'}
              </Button>
            </div>
          </div>
        </div>
